...
def extract_pitch(audio):
    try:
        ...
        return np.mean(pitches), pitches[-1] - pitches[0]
    except:
        return 0, 0
...
def harmonic_energy(audio):
    ...
    return np.mean(librosa.feature.rms(y=harmonic))

# =========================
# Human vs Baby Detection
# =========================

def detect_human_vs_baby(audio):

    pitch_mean, _ = extract_pitch(audio)
    rms = float(np.mean(librosa.feature.rms(y=audio)))
    centroid = float(np.mean(librosa.feature.spectral_centroid(y=audio, sr=SR)))

    if pitch_mean < 200 and centroid < 2500:
        return "Human Voice"
...
def predict_from_full_audio(full_audio):
    ...
    for start in range(0, len(full_audio)-window+1, window):
    ...
    for _, start in energies[:3]:
        ...
        asphyxia_index = list(asphyxia_model.classes_).index(1)
    ...
    if len(probs_list) == 0:
        return 0, full_audio[:window]

    return np.mean(probs_list), segments[np.argmax(probs_list)]
...
def predict():
    ...
    try:
        ...
        # Human check
        voice_type = detect_human_vs_baby(full_audio)

        if voice_type == "Human Voice":
            return jsonify({
                "classification": "Not a baby cry",
                "confidence": 0
            })

        if voice_type == "Unclear":
            return jsonify({
                "classification": "Unclear Audio",
                "confidence": 0
            })
        ...
        # Asphyxia
        prob, segment = predict_from_full_audio(full_audio)

        if prob >= ASPHYXIA_THRESHOLD:
            return jsonify({
        ...
        score = (csi - csi_min) / (csi_max - csi_min)
        score = max(0, min(1, score))

        cause = guess_possible_cause(segment)
        ...
        print(e)
        return jsonify({"classification":"Error","confidence":0})
